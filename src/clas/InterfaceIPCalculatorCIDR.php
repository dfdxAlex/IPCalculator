...
class InterfaceIPCalculatorCIDR
{
    ...
    public function interfaceIPCalculatorCIDR()
    {
        if ($this->interfaceTrueFalse->showUserMenu()) ;
            ...
            echo '<p>Десятичная маска подсети: '.$this->maska->maska10(
                                                     $this->maska->maska2()
                                                     ).'</p>';

            echo '<p>Адрес сети: '.$this->network->networkAddressByMaskAndIp(
                                       $this->ip10To2->ip10To2($this->ip),
                                       $this->maska->maska2()
                                       ).'</p>';

            echo '<p>Первый адрес в сети: '.$this->first->firstAddress(
            ...
            echo '<p>Последний адрес в сети: '.$this->first->firstAddress(
                                                   $this->network->networkAddressByMaskAndIp(
                                                        $this->ip10To2->ip10To2($this->ip),
                                                        $this->maska->maska2()
                                                    ),
            ...
            echo '<p>Широковещательный адрес в сети: '.$this->first->firstAddress(
                                                           $this->network->networkAddressByMaskAndIp(
                                                               $this->ip10To2->ip10To2($this->ip),
                                                               $this->maska->maska2()),
